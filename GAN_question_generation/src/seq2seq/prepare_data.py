...
def prepare_sequence(seq, word2index, max_len):
    sequence = [word2index[w] if w in word2index else word2index['<unk>'] for w in split_text(seq)[:max_len - 1]]
...
def prepare_pq_sequence(post_seq, ques_seq, word2index, max_post_len, max_ques_len):
    p_sequence = [word2index[w] if w in word2index else word2index['<unk>'] for w in
                  split_text(post_seq)[:(max_post_len - 1)]]
    p_sequence.append(word2index[EOP_token])
    p_sequence += [word2index[PAD_token]] * (max_post_len - len(p_sequence))
    q_sequence = [word2index[w] if w in word2index else word2index['<unk>'] for w in
                  split_text(ques_seq)[:(max_ques_len - 1)]]
