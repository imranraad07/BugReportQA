...
from calculator_avg import compute_utilities
...
def create_tsv_files_github(args, post_titles, post_texts, post_questions, post_answers):
    lucene_similar_posts = get_similar_docs(args.lucene_similar_posts)
    similar_posts = {}
    ...
    for postId in similar_posts:
        with open(args.post_data_tsv, 'a') as out_file:
            ...
            with open(args.qa_data_tsv, 'a') as out_file:
                tsv_writer = csv.writer(out_file, delimiter='\t')
                tsv_writer.writerow(row_val)

    compute_utilities(args.answer_ob_eb_s2r_csv, args.data_avg_tsv, args.post_data_tsv, args.qa_data_tsv, qa_post_ids,
                      args.utility_data_tsv)
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--answer_ob_eb_s2r_csv", type=str)
    argparser.add_argument("--data_avg_tsv", type=str)
