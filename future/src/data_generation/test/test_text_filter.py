...
from future.src.data_generation.github_text_filter import filter_nontext as filter
...
class TestRecPatterns(unittest.TestCase):
    ...
    def test_triple_quotes_multiple_triple_quote_eof(self):
        br = 'something ```chasing``` ```regarding``` something ```code \n code \n' + \
             'code\n' + \
             'code'
        text = filter(br)
