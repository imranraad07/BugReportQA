...
def generate_data(posts, ques_list, ans_list, args):
    ...
    for i in range(data_size):
        data_posts[i] = padding(posts[i], args.post_max_len)
        for j in range(N):
            try:
                data_ques_list[i][j] = padding(ques_list[i][j], args.ques_max_len)
            except:
                count_except = count_except + 1
            try:
                data_ans_list[i][j] = padding(ans_list[i][j], args.ans_max_len)
            except:
                count_except2 = count_except2 + 1

    print str(count_except), str(count_except2)
    return data_posts, data_ques_list, data_ans_list
...
def evaluate_using_human_annotations(args, preds):
    ...
    print
    print '\t\tBest acc on 10: %.2f  Valid acc on 10: %.2f' % (
        best_acc_on10 * 100.0 / total, valid_acc_on10 * 100.0 / total)
    print '\t\tBest acc on 9:  %.2f  Valid acc on 9:  %.2f' % (
        best_acc_on9 * 100.0 / total_best_on9, valid_acc_on9 * 100.0 / total)
