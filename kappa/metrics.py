...
from kappa.utils import convert_dataframe
...
class Metrics():
    ...
    def correlation(self, ann1, ann2, measure="pearson"):
        ...
        if (len(ann1_) == 0 and len(ann2_) == 0):
            raise ValueError(
                "Annotators " + str(ann1) + " and " + str(ann2) + " have not labelled any of the same instances.")
...
class Krippendorff():
    ...
    def __init__(self, df, use_tqdm=False):
        ...
        self.coincidence_matrix = coincidence_mat(self.A, self.labels, self.num_anns, self.num_instances,
                                                  self.labels_per_instance, self.use_tqdm)
