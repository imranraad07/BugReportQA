...
from GitHub issues, including their corresponding bug reports and answers. When the bug report is deficient in OB, EB, or S2R, as output, \evpi poses a single (or a small set) follow-up question appropriate to the deficient bug report. 
...
In more detail, we used the following sequence of steps to curate the corpus. The highlights of the corpus curation process are also illustrated on the right part of Figure~\ref{fig:pipeline} (Offline Corpus Curation Workflow).
\begin{enumerate}
...
in projects on GitHub, which is why we opted to include unlabeled issues. Since we are interested in deficient bug reports, we selected bug reports that do not contain Observable Behavior, Expected Behavior or Steps to Reproduce. While focusing separately on incompleteness in one of these categories is a possible strategy, for simplicity, we chose only bug reports that lacked all three.
