...
as the product of: 1)the appropriateness of a specific follow up question to the bug report, i.e.,
...
The utility of a question is further expressed in terms of the average quality of answers it has received, i.e.,
...
network approximation, consisting of a deep NN consisting of 3 layers. As the first layer we encode
the bug report text with GloVE embeddings trained on the entirety of Stack Overflow posts. As the second layer we train
a LSTM and compute an average of the hidden layer. As the third and final layer we use a dense neural network consisting
of 4 inter-connected layers. We train this neural architecture using the actually posed follow-up questions and answers, which we concatenate, as positive labeled examples and others in the candidate set as negatively labeled. We use weighing to manage the resulting class imbalance as the negative examples significantly outnumber the positive ones.
...
the authors. Each of the 15 patterns rely on predefined sets of keywords and part-of-speech tagging to ensure high precision.
In order to compute the average utility of the answers for a particular question in the corpus, we rely on Lucene to provide matching
between questions and we reserve 10 of the most similar question variants for each queried follow-up question.
